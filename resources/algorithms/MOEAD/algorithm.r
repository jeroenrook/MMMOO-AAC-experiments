...
option_list = list(
  ...
  make_option("--neighborsT", type = "numeric", default = 100L, help="Neighborhood size. Msut be smaller than the number of subproblems"),
  make_option("--neighbors_delta_p", type = "numeric", default = 0.5, help="Probability of sampling from the neighborhood when performing variation. Must be a scalar value between 0 and 1."),
  make_option("--decomp", type = "character", default = "sld", help="[msld, sld, uniform]"),
  make_option("--decompH", type = "numeric", default = 99L, help="decomposition constant. vectors generated (N) must be greater than the number declared in neighbors$T"),
  make_option("--aggfun", type = "character", default = "awt", help="[awt, ipbi, pbi, ws, wt]"),
...
if(opt$preset != "custom"){
  ...
  if (opt$decomp == "uniform"){
    ...
    decomp    <- list(name       = opt$decomp, H = opt$decompH)
  } else{
