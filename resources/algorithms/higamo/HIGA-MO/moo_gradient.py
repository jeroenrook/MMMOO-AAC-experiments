...
class MOO_HyperVolumeGradient:
    ...
    def __init__(self, dim_d, dim_o, lb, ub,
        ...
        self.enable_dominated = True if 'enable_dominated' not in kwargs \
            else kwargs['enable_dominated']
        self.maxiter = kwargs['maxiter'] if 'maxiter' in kwargs else inf
    ...
    def __nearst_gap(self, mid_gap, idx):
        ...
        if len(mid_gap) != 0:
            if mid_gap.shape[1] > 1:
                if 1 < 2:
                    avail_gap = list(range(mid_gap.shape[1]))
                    for i in idx:
                        if len(avail_gap) == 0:
                            avail_gap = list(range(len(mid_gap)))
        ...
        if self.itercount != 0:
            ...
            if 11 < 2:
                if self.dominated_steer == 'NDS':
                    control_set = list(range(self.mu))
                else:
