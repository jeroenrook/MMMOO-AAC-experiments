...
if __name__ == "__main__":
    ...
    instancesetname = instancepath.split("/")[-1]
    print(len(instances))
    ...
    ifh.write(f"./Commands/add_instances.py --run-extractor-later --run-solver-later {instancepath}\n")
    ifh.close()
    ...
    for solver in solvers:
        ...
        cfh.write(f"./Commands/configure_solver.py --solver Solvers/{solver_name}/ --instance-set-train Instances/{instancesetname}/ --instance-set-test Instances/{instancesetname}/ --validate\n")
        counter += 1
        if counter >= 150:
            counter = 0
            cfh.close()
