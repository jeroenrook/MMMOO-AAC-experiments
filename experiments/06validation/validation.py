...
if __name__ == "__main__":
    ...
    for i in range(args.runs):
        ...
        tmp_file = f"{TMP}/data_{filehash}.RData"
        ...
        try:
            ...
            # if "solution_set" in result and args.visualise is not None and i == 0:
            #     print(result["solution_set"])
            #     cols = result["solution_set"].columns
            #     plt.scatter(result["solution_set"][cols[0]], result["solution_set"][cols[1]])
            #     plt.savefig(args.visualise)

            #Get data
            # if args.target in result:
            #     print(result[args.target].iloc[0][args.target])
            #     performances[i] = abs(float(result[args.target].iloc[0][args.target]))
            # else:
            #     raise Exception(f"{args.target} is not available")
            metrics = ["HV", "SPD", "HVN"]
            for target in metrics:
                if target in result:
                    row[target] = abs(float(result[target].iloc[0][target]))
                else:
                    row[target] = None
